...
\begin{tabular}{|p{1.5cm}|p{6.6cm}|}
 ...
 if(command_line->HasSwitch(
     s::kTestLauncherTotalShards) &&
     command_line->HasSwitch(
         s::kTestLauncherShardIndex)
     ){
 if(!StringToInt(
      command_line
      ->GetSwitchValueASCII(
 ...
 Consequences & If this smell is observed it will have the following consequences in the project
Cost of testing will increase so as to cover all the code paths.
Changing existing code base will become non-trivial.
Code complexity is higher, difficult to understand.
 \\ \hline
...
\begin{figure*}
\includegraphics[height=6in,width=7in]{figure/uml.png}
...
\textbf{Toggle method signature or function name:} Without this information the toggle smell detector could not search for the pattern of the method used to check the toggle usage. Figure~\ref{fig:uml} represents the design of our 
approach in an UML diagram.
