...
concept but recently being practice in many software companies. Improper 
...
Mix of Compile-Time and Runtime Toggles, Legacy/Stable toggles and Spread Toggles. 
...
\subsection{TOGGLE\_SPAGHETTI (Combinatorial toggles)}

\begin{table}[ht]
\caption{Combinatorial Toggle Smell}
\centering
\begin{tabular}{|p{1.3cm}|p{7cm}|}
 \hline\hline
 Description & This smell is observed when the execution of code is controlled by combination of states of several toggles.  \\ \hline 
 Example &  
 \begin{lstlisting} 
 	//Location: /chrome/browser/about_flags.cc (taken from Google chrome)
void FlagsState::
ConvertFlagsToSwitches(FlagsStorage* flags_storage, 
base::CommandLine* command_line, SentinelsMode sentinels) {
if (command_line->
HasSwitch(switches::kNoExperiments))
return;

std::set<std::string> enabled_experiments;
//FlagsForCurrentPlatform = FFCP
GetSanitizedEnabledFFCP
(flags_storage, &enabled_experiments);
...
if (e.type == Experiment::SINGLE_VALUE) {
	...
	e.internal_name, e.command_line_switch,
	e.command_line_value, &name_to_switch_map
		...
		SetFlagToSwitchMapping(
		e.NameForChoice(0), std::string(), std::string(),
		&name_to_switch_map
		);

		SetFlagToSwitchMapping(
		e.NameForChoice(1), e.command_line_switch,
		e.command_line_value, &name_to_switch_map
		);
		SetFlagToSwitchMapping(
		e.NameForChoice(2), e.disable_command_line_switch,
 ...
 \begin{enumerate}
\item Mix of canary use cases or A/B testing in toggle block could result in this kind of spaghetti code.
\item Inconsistency in API’s used to manage toggles and canary use cases.
\item Presence of more than one API to manage feature flags. Transition from legacy feature toggles to more formal one.
\end{enumerate}
...
\subsection{Mix of Compile time and run-time toggles)}
...
\begin{tabular}{|p{1.5cm}|p{7cm}|}
 ...
 Example & 
 \begin{lstlisting}
 //Location: /chrome/app/chrome_main_delegate.cc (Taken from Google chrome)
